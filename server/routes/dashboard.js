...
router.get('/', authenticate, async (req, res) => {
  try {
    // Fetch user data first (needed for animal query)
    ...
    const [animalResult, itemsResult, recentResult] = await Promise.all([
      user.selected_animal_id
        ? pool.query(`
    ...
    const animal = animalResult.rows[0] || null;
